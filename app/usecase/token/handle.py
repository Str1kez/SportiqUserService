...
async def create_token(user_id: str, _type: Literal["access", "refresh"]) -> str:
    token = _create_token(user_id, _type)
    return await _encrypt_token(token)
...
def _create_token(user_db_id: str, _type: Literal["access", "refresh"]) -> Token:
    settings = JWTSettings()
    issued = datetime.utcnow()
    expire = issued + (settings.access_expires if _type == "access" else settings.refresh_expires)
    jti = str(uuid4())
